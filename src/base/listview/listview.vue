<template>
  <scroll @scroll="scroll" class="listview" :data='data' :listen-scroll='listenScroll' :probe-type="probeType" ref="listview">
    <ul>
      <li v-for="(group,i) in data" class="list-group" ref="listgroup" :key="i">
        <h2 class="list-group-title">{{group.title}}</h2>
        <ul>
          <li v-for="(item,i) in group.items" @click="selectItem(item)" class="list-group-item" :key="i">
    ...
    <div class="list-shortcut" @touchstart.stop.prevent="onShortcutTouchStart" @touchmove.stop.prevent="onShortcutTouchMove" @touchend.stop>
      <ul>
        <li v-for="(item,index) in listShortcut" class="item" :data-index="index" :class="{'current':currentIndex===index}" :key="index">
          {{item}}
        </li>
      </ul>
    </div>
